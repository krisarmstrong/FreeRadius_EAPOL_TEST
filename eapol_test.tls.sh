...
do
	...
	# Replacing client_cert= & private_key= with new key and cert
    	sed -i "/client_cert=/c\ \tclient_cert=\"$i\"" eapol_test.conf.tls
	sed -i "/private_key=/c\ \tprivate_key=\"$i\"" eapol_test.conf.tls 
	if eapol_test -c eapol_test.conf.tls -a$1 -p$2 -s $3 -r1 | grep -q 'SUCCESS'; then
